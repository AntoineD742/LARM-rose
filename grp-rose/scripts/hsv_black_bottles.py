...
# hiOrangeBottle = np.array([25, 255, 255])   #Bouteille orange (Max)

lowBlack = np.array([0, 0, 0])   #Bouteille orange (Min)
hiBlack = np.array([179, 200, 200])   #Bouteille orange (Min)
...
class bottleFinder: 
    ...
    def find_bottles(self):
        if self.depth_map is not None and self.color_map is not None:
            ...
            #Mask noir
            maskNoir = cv2.inRange(img_hsv, lowBlack, hiBlack)
            maskNoir=cv2.erode(maskNoir, None, iterations=3)
            maskNoir=cv2.dilate(maskNoir, None, iterations=3)
            kernel = np.ones((3,3), np.uint8)
            maskNoir = cv2.morphologyEx(maskNoir, cv2.MORPH_CLOSE, kernel)
            ...
            img_result=cv2.bitwise_and(self.color_map, self.color_map, mask= maskNoir)
            grey = cv2.cvtColor(img_result, cv2.COLOR_BGR2GRAY)
            # print(rect)
            # cv2.rectangle(img_result, rect,	(0,255,0),3)
            contours, hierarchy = cv2.findContours(grey, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
            for cntr in contours:
                # get bounding boxes
                rect = cv2.boundingRect(cntr)
                x,y,w,h = cv2.boundingRect(cntr)
                crop = img_hsv[y:y+h, x:x+w]
                maskBadge=cv2.inRange(crop, lowBadge, hiBadge)
                maskBadge=cv2.erode(maskBadge, None, iterations=1)
                maskBadge=cv2.dilate(maskBadge, None, iterations=1)
                crop = cv2.bitwise_and(crop, crop, mask= maskBadge)
                grayCounter = cv2.cvtColor(img_result, cv2.COLOR_BGR2GRAY)
                nbrPixelsDetectes = cv2.countNonZero(grayCounter)
                if nbrPixelsDetectes > 1000:
                    cv2.rectangle(self.color_map, rect,	(0,255,0),3)
